...
// Since elasticsearch is just a key/value store, we'll use key suffixes
...
static char *elasticsearch_backend__build_key(const unsigned char *id, const char *suffix, size_t *out_len)
{
	char *new_key;
...
/* 	assert(len_p && type_p && _backend && oid);
	...
	type_key = elasticsearch_backend__build_key(oid->id, type_suffix, &type_key_len);
	if (type_key == NULL)
		return GIT_ENOMEM;

	size_key = elasticsearch_backend__build_key(oid->id, size_suffix, &size_key_len);
	if (size_key == NULL)
		return GIT_ENOMEM;


	memset(type_p, 0, sizeof(type_p));
	type_p = (git_otype *)elasticsearch_get(backend->db, type_key, type_key_len, &type_value_len, &type_flags, &ret);
	if (type_p == NULL) {
	...
	size_value = (size_t *)elasticsearch_get(backend->db, size_key, size_key_len, &size_value_len, &size_flags, &ret);
	if (size_value == NULL) {
		if (type_p)
			free(type_p);
...
/* 	assert(data_p && len_p && type_p && _backend && oid);
	...
	data_key = elasticsearch_backend__build_key(oid->id, data_suffix, &data_key_len);
	if (data_key == NULL)
		return GIT_ENOMEM;


	type_buffer = (void *)elasticsearch_get(backend->db, type_key, type_key_len, &type_value_len, &type_flags, &ret);
	if (type_buffer == NULL) {
		status = GIT_ENOTFOUND;
		goto read_cleanup;
	}

	data_buffer = (void *)elasticsearch_get(backend->db, data_key, data_key_len, len_p, &data_flags, &ret);
...
/* 
	...
	ret = elasticsearch_add(backend->db, type_key, type_key_len, "", 0, 0, 0);
	if (ret == elasticsearch_DATA_EXISTS) {
		// object exists
		found = 1;
	}
...
/* 
	...
	ret = elasticsearch_set(backend->db, type_key, type_key_len, (const char *)&type, sizeof(type), 0, 0);
	if (ret != elasticsearch_SUCCESS) {
		status = GIT_ERROR;
		goto write_cleanup;
	}

	ret = elasticsearch_set(backend->db, size_key, size_key_len, (const char *)&len, sizeof(len), 0, 0);
	if (ret != elasticsearch_SUCCESS) {
		status = GIT_ERROR;
		goto write_cleanup;
	}

	ret = elasticsearch_set(backend->db, data_key, data_key_len, (const char *)data, len, 0, 0);
	if (ret != elasticsearch_SUCCESS) {
		status = GIT_ERROR;
		goto write_cleanup;
	}
...
/* 	elasticsearch_backend *backend;
	...
	if (backend->db)
		elasticsearch_free(backend->db);

	free(backend); */
}

int git_odb_backend_elasticsearch(git_odb_backend **backend_out, const char *host, int port)
{
/* 	elasticsearch_backend *backend;
	elasticsearch_return ret = 0;
	uint64_t set = 1;
	...
	backend->db = elasticsearch_create(NULL);
	if (backend->db == NULL)
		goto cleanup;

	ret = elasticsearch_server_add(backend->db, host, port);
	if (ret != elasticsearch_SUCCESS)
		goto cleanup;

	// requires elasticsearch 1.3+
	elasticsearch_behavior_set(backend->db, elasticsearch_BEHAVIOR_BINARY_PROTOCOL, set);

	elasticsearch_behavior_set(backend->db, elasticsearch_BEHAVIOR_NO_BLOCK, set);
	elasticsearch_behavior_set(backend->db, elasticsearch_BEHAVIOR_TCP_NODELAY, set);
