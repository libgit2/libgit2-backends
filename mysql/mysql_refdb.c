...
} mysql_refdb_backend_t;


typedef struct mysql_reference_iterator_node_s {
    struct git_reference *reference;
    int must_free_ref;
    struct mysql_reference_iterator_node_s *next;
} mysql_reference_iterator_node_t;


typedef struct {
    git_reference_iterator parent;
    mysql_reference_iterator_node_t *head;
    mysql_reference_iterator_node_t *current;
} mysql_reference_iterator_t;


static int mysql_reference_iterator_next(git_reference **ref,
        git_reference_iterator *iter);
static int mysql_reference_iterator_next_name(const char **ref_name,
        git_reference_iterator *iter);
static void mysql_reference_iterator_free(git_reference_iterator *iter);


/*!
 * \brief Template for our mysql_reference_iterator_t.
 * When creating, we can memcpy() this template, and just fill in the taggued
 * fields.
 */
static const mysql_reference_iterator_t reference_iterator_template = {
    .parent = {
        .db = NULL,  /* To fill in */
        .next = mysql_reference_iterator_next,
        .next_name = mysql_reference_iterator_next_name,
        .free = mysql_reference_iterator_free,
    },
};
...
{
    mysql_reference_iterator_t *iterator;

    assert(ref);
    assert(_iterator);

    iterator = (mysql_reference_iterator_t *)_iterator;
...
static void mysql_reference_iterator_free(git_reference_iterator *_iterator)
{
    mysql_reference_iterator_t *iterator;
    mysql_reference_iterator_node_t *node, *nnode;
...
static int mysql_refdb_exists(int *exists, git_refdb_backend *_backend,
        const char *refname)
{
    mysql_refdb_backend_t *backend;
    MYSQL_BIND bind_buffers[2];

    backend = (mysql_refdb_backend_t *)_backend;
...
static int mysql_refdb_lookup(git_reference **out,
...
static int mysql_refdb_iterator(git_reference_iterator **_iterator,
        struct git_refdb_backend *_backend, const char *glob)
{
    ...
    int fetch_result;
    mysql_reference_iterator_t *iterator;
    mysql_reference_iterator_node_t *current_node = NULL;
    mysql_reference_iterator_node_t *previous_node = NULL;
    ...
    *_iterator = NULL;
    ...
    iterator->parent.next = mysql_reference_iterator_next;
    iterator->parent.next_name = mysql_reference_iterator_next_name;
    iterator->parent.free = mysql_reference_iterator_free;
    ...
    if(mysql_stmt_bind_result(backend->st_iterator, result_buffers) != 0) {
        fprintf(stderr, __FILE__ ": %s: L%d: "
            ...
            mysql_stmt_error(backend->st_iterator));
        mysql_stmt_reset(backend->st_exists);
        mysql_reference_iterator_free(&iterator->parent);
        return GIT_EUSER;
    }
 ...
 */
static int mysql_refdb_write(git_refdb_backend *_backend,
        ...
        const git_oid *old, const char *old_target)
{
    ...
    if (mysql_stmt_execute(sql_statement) != 0) {
        ...
        if (!force) {
            ...
            int err, exists = 0;
            err = mysql_refdb_exists(&exists, _backend, ref_name);
            if (err == 0 && exists) {
                return GIT_EEXISTS;
            }
        }
...
static int mysql_refdb_del(git_refdb_backend *backend, const char *ref_name,
        const git_oid *old_id, const char *old_target);

static int mysql_refdb_rename(git_reference **out,
        git_refdb_backend *_backend,
        ...
        const git_signature *who, const char *message)
{
    ...
    *out = NULL;
    ...
    if (force) {
        ...
        if (mysql_refdb_exists(&exists, _backend, new_name) != GIT_OK) {
            /* it already set a python exception for us */
            return GIT_EUSER;
        }

        if (exists
                && (mysql_refdb_del(_backend, new_name, NULL, NULL)
                    != GIT_OK)) {
            /* it already set a python exception for us */
            return GIT_EUSER;
        }
    ...
    return mysql_refdb_lookup(out, _backend, new_name);
}


static int mysql_refdb_del(git_refdb_backend *_backend, const char *ref_name,
        const git_oid *old_id, const char *old_target)
{
    /* XXX(JFlesch):
    ...
    if (mysql_stmt_bind_param(backend->st_delete, bind_buffers) != 0) {
        fprintf(stderr, __FILE__ ": %s: L%d: "
                "mysql_stmt_bind_param() failed: %s",
                __FUNCTION__, __LINE__,
                mysql_stmt_error(backend->st_delete));
        return GIT_EUSER;
    }
    ...
    if (affected_rows != 1) {
        fprintf(stderr, __FILE__ ": %s: L%d: "
                ...
                __FUNCTION__, __LINE__,
                mysql_stmt_error(backend->st_delete), affected_rows);
        return GIT_ENOTFOUND;
    }
...
static int mysql_refdb_compress(git_refdb_backend *_backend)
{
    mysql_refdb_backend_t *backend;
    ...
    if (mysql_stmt_execute(backend->st_optimize) != 0) {
        fprintf(stderr, __FILE__ ": %s: L%d: "
                "mysql_stmt_execute() failed: %s",
                __FUNCTION__, __LINE__,
                mysql_stmt_error(backend->st_optimize));
        return GIT_EUSER;
    }
...
static int mysql_refdb_lock(void **payload_out, git_refdb_backend *backend,
        const char *refname)
{
...
static int mysql_refdb_unlock(git_refdb_backend *backend, void *payload,
...
static void mysql_refdb_free(git_refdb_backend *_backend)
{
    mysql_refdb_backend_t *backend;
...
static int mysql_refdb_has_log(git_refdb_backend *backend,
        const char *refname)
{
...
static int mysql_refdb_ensure_log(git_refdb_backend *backend,
        const char *refname)
{
...
static int mysql_refdb_reflog_read(git_reflog **out,
...
static int mysql_refdb_reflog_write(git_refdb_backend *backend,
        git_reflog *reflog)
{
    /* We don't use reflogs */
    backend = backend; /* -Wunused-parameter */
...
static int mysql_refdb_reflog_rename(git_refdb_backend *backend,
...
static int mysql_refdb_reflog_delete(git_refdb_backend *backend,
        const char *refname)
{
    /* We don't use reflogs */
    backend = backend; /* -Wunused-parameter */
...
static int init_statements(mysql_refdb_backend_t *backend,
        const char *mysql_table)
{
        ...
        int refdb_partitions)
{
    ...
    backend = calloc(1, sizeof(mysql_refdb_backend_t));
    if (backend == NULL) {
        return GIT_EUSER;
    }
    ...
    *backend_out = &backend->parent;
    ...
    error = init_db(db, mysql_table, refdb_partitions);
    if (error < 0) {
        goto error;
    }

    error = init_statements(backend, mysql_table);
    if (error < 0) {
        goto error;
    }
    ...
    backend->parent.exists = mysql_refdb_exists;
    backend->parent.lookup = mysql_refdb_lookup;
    backend->parent.iterator = mysql_refdb_iterator;
    backend->parent.write = mysql_refdb_write;
    backend->parent.rename = mysql_refdb_rename;
    backend->parent.del = mysql_refdb_del;
    backend->parent.compress = mysql_refdb_compress;
    backend->parent.has_log = mysql_refdb_has_log;
    backend->parent.ensure_log = mysql_refdb_ensure_log;
    backend->parent.free = mysql_refdb_free;
    backend->parent.reflog_read = mysql_refdb_reflog_read;
    backend->parent.reflog_write = mysql_refdb_reflog_write;
    backend->parent.reflog_rename = mysql_refdb_reflog_rename;
    backend->parent.reflog_delete = mysql_refdb_reflog_delete;
    backend->parent.lock = mysql_refdb_lock;
    backend->parent.unlock = mysql_refdb_unlock;

    return GIT_OK;

error:
    mysql_refdb_free(&backend->parent);
    return GIT_EUSER;
}
