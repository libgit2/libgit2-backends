...
#define UNIMPLEMENTED_CALLBACK(cb_name) static int cb_name() \
    { \
        fprintf(stderr, \
            "mysql ODB: WARNING: %s called but not implemented !\n", \
            __FUNCTION__); \
        assert(0); \
        return GIT_EUSER; \
    }
...
typedef struct {
     ...
     * we load everything in memory, and we call mysql_odb_backend__write()
     * on it ...
     */
    char *buffer;
    ...
    size_t written;
    size_t total;
} mysql_odb_writestream_t;


static int mysql_odb_backend__read_header(size_t *len_p, git_otype *type_p,
        git_odb_backend *_backend, const git_oid *oid)
{
    mysql_odb_backend_t *backend;
    int error;
    ...
    backend = (mysql_odb_backend_t *)_backend;
...
static int mysql_odb_backend__read(void **data_p, size_t *len_p,
...
static int mysql_odb_backend__read_prefix(
    ...
    *data_p = NULL;
    ...
    if (len >= GIT_OID_HEXSZ) {
        ...
        return mysql_odb_backend__read(data_p, len_p, type_p, _backend,
            short_oid);
    }
...
static int mysql_odb_backend__exists_prefix(
        ...
        const git_oid *short_oid, size_t len)
{
    ...
    if (len >= GIT_OID_HEXSZ) {
        ...
        memcpy(out_oid, short_oid, sizeof(*out_oid));
        return mysql_odb_backend__exists(_backend, short_oid);
    }
...
static int mysql_odb_backend__write(git_odb_backend *_backend,
    const git_oid *oid, const void *data, size_t len, git_otype type)
{
    ...
    if (mysql_stmt_bind_param(backend->st_write, bind_buffers) != 0) {
        fprintf(stderr, __FILE__ ": %s: L%d: "
                "mysql_stmt_bind_param() failed: %s",
                __FUNCTION__, __LINE__,
                mysql_stmt_error(backend->st_write));
        return GIT_EUSER;
    }
...
static int mysql_odb_backend__refresh(git_odb_backend *backend) {
...
UNIMPLEMENTED_CALLBACK(mysql_odb_backend__readstream)

/* int foreach(git_odb_backend *, git_odb_foreach_cb cb, void *payload); */
UNIMPLEMENTED_CALLBACK(mysql_odb_backend__foreach)

/* int writepack(git_odb_writepack **, git_odb_backend *, git_odb *odb,
 * git_transfer_progress_cb progress_cb, void *progress_payload);
 */
UNIMPLEMENTED_CALLBACK(mysql_odb_backend__writepack)

#pragma GCC diagnostic pop


static int mysql_odb_backend__writestream_write(git_odb_stream *_stream,
        const char *buffer, size_t len)
{
    mysql_odb_writestream_t *stream = (void *)_stream;
...
static int mysql_odb_backend__writestream_finalize_write(
        git_odb_stream *_stream, const git_oid *oid)
{
    ...
    return mysql_odb_backend__write(stream->parent.backend,
        oid, stream->buffer, stream->written, stream->otype);
}

static void mysql_odb_backend__writestream_free(git_odb_stream *_stream)
{
    mysql_odb_writestream_t *stream = (void *)_stream;
...
static int mysql_odb_backend__writestream(git_odb_stream **out_stream,
    git_odb_backend *_backend, size_t len, git_otype otype)
{
    mysql_odb_writestream_t *stream;
    mysql_odb_backend_t *backend = (void *)_backend;
    ...
    stream = calloc(1, sizeof(mysql_odb_writestream_t));
    if (stream == NULL) {
        return GIT_EUSER;
    }
    ...
    stream->parent.write = mysql_odb_backend__writestream_write;
    stream->parent.finalize_write = \
            mysql_odb_backend__writestream_finalize_write;
    stream->parent.free = mysql_odb_backend__writestream_free;
...
static int init_statements(mysql_odb_backend_t *backend, const char *mysql_table)
{
    my_bool truth = 1;
        ...
        *(statements[st_idx].stmt) = mysql_stmt_init(backend->db);
        ...
        if (mysql_stmt_attr_set(*(statements[st_idx].stmt),
            ...
            fprintf(stderr, __FILE__
                ...
                mysql_stmt_error(*(statements[st_idx].stmt)));
            return GIT_EUSER;
        }
        ...
        int odb_partitions)
{
    ...
    backend = calloc(1, sizeof(mysql_odb_backend_t));
    if (backend == NULL) {
        return GIT_EUSER;
    }
    ...
    error = init_db(db, mysql_table, odb_partitions);
    if (error < 0) {
        goto cleanup;
    }
    ...
    backend->parent.writestream = mysql_odb_backend__writestream;
    backend->parent.readstream = mysql_odb_backend__readstream;
    backend->parent.exists = mysql_odb_backend__exists;
    backend->parent.exists_prefix = mysql_odb_backend__exists_prefix;
    backend->parent.refresh = mysql_odb_backend__refresh;
    backend->parent.free = mysql_odb_backend__free;
    backend->parent.foreach = mysql_odb_backend__foreach;
    backend->parent.writepack = mysql_odb_backend__writepack;
...
cleanup:
    mysql_odb_backend__free(&backend->parent);
    return GIT_EUSER;
}
